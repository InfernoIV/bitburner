...
export class hack_obj {
    ...
    async manage(ns, handles) {
        ...
        if (this.hack_target == "") {
            ...
            this.time_of_next_check = Date.now() + time_wait + this.time_between
        }
    }
    ...
    */
    calculate_timings(ns, server, player) {
        ...
        this.delay = {
            grow: {
                grow: this.time.weaken - this.time.grow - this.time_between,
                weaken: 0,
            },
            hack: {
                hack: this.time.weaken - this.time.hack - this.time_between,
                weaken_1: 0,
                grow: this.time.weaken - this.time.grow - this.time_between,
                weaken_2: (2 * this.time_between),
            },
        }
    }
    ...
    */
    ...
    weaken_server(ns, execute_servers) {
        ...
        return this.time.weaken + this.time_between
    }
        ...
        */
        ...
        for (const [server, ram_available] of execute_servers) {
            ...
            if (job_size < ram_available) {
                ...
                //increase job delay
                job_delay += (2 * this.time_between)
            }
        }
        //return time to wait
        return this.time.weaken + job_delay - this.time_between
    }
