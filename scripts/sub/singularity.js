...
export async function backdoor(ns, neighbour, server) {
...
export async function print_faction_requirements(ns) {
	...
	for (const faction of FactionNameEnumType) { //or FactionName?
		...
		//print information
		ns.tprint("faction '" + faction + "' requires: '" + JSON.stringify(requirements) + "'")
	}
}
...
export async function perform_destruction(ns) {
	...
	if (ns.singularity.getOwnedAugmentations(purchased).includes("The Red Pill")) { 
		...
		const required_hacking_level = await evaluate.exec(ns, "ns.getBitNodeMultipliers()").WorldDaemonDifficulty * 5000
				...
				for (const work_type of work_types) {
					...
					switch (work_type) {
						...
						case FactionWorkType.field: 
							...
							stat = (0.9 * (player.skills.strength + player.skills.defense + player.skills.dexterity + player.skills.agility + player.skills.hacking)) / 4.5)
							break
						case FactionWorkType.security: 
							...
							stat = ((0.9 * (player.skills.strength + player.skills.defense + player.skills.dexterity + player.skills.agility + player.skills.hacking + player.skills.charisma)) / 5.5)
							break
						default: break
					}
...
async function perform_action(ns, player, work_type, activity, group = "") {
	...
	if (await evaluate.exec(ns, "ns.singularity.isBusy()")) {
		...
		if (current_task.type == work_type) {
			//what to check?
			switch (work_type) {
				case work_type.GRAFTING: 		return //finish crafting
				case work_type.FACTION:			if (current_task.factionName == group && current_task.factionWorkType == activity) { return } break
				case work_type.COMPANY:			if (current_task.companyName == group) { return } break //only look at the company?
				case work_type.CRIME:			if (current_task.activity == activity) { return } break //only look at the crime
				case work_type.STUDY: 			if (current_task.classType == activity) { return } break //only look at the type (location doesn't matter)
				case work_type.CREATE_PROGRAM:	return //finish program 	if (current_task.programName == activity) { return }
				default: 						ns.tprint("Uncaught case: " + work_type) break
			}
		//we want to do something different but we are grafting or creating a program
		} else if (current_task.type == work_type.GRAFTING || current_task.type == work_type.CREATE_PROGRAM) {
			//don't change
			return
		}		
	}
	...
	switch (work_type) {
		case work_type.FACTION:			await evaluate.exec(ns, "ns.singularity.workForFaction('" + group + "','" + activity + "'," + focus + ")") break
		case work_type.COMPANY:			await evaluate.exec(ns, "ns.singularity.workForCompany('" + group + "'," + focus + ")") break
		case work_type.CRIME:			await evaluate.exec(ns, "ns.singularity.commitCrime('" + activity + "'," + focus + ")") break
		case work_type.STUDY:			await evaluate.exec(ns, "ns.singularity.universityCourse('" + group + "','" + activity + "'," + focus + ")") break//universityName, courseName, focus)
		case work_type.CREATE_PROGRAM:	await evaluate.exec(ns, "createProgram('" + activity + "'," + focus + ")") break
		case work_type.GRAFTING:	break //TODO
	
		default: ns.tprint("Uncaught case: " + work_type)
	}
}
