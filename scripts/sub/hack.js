...
export class hack_obj {
    ...
    async manage(ns, handles) {
        ...
        //get servers with money (available)
        const servers_money = handles[CONSTANTS.HANDLE.ROOT].servers_money
        //get player information
        const player = ns.getPlayer()
        //get target
        this.find_target(ns, servers_money, player, handles.hasOwnProperty(CONSTANTS.HANDLE.INTELLIGENCE))
    ...
    */
    find_target(ns, servers_money, player, formulas_available) {
        ...
        if (hacking_level_current > this.hack_level_previous) {
            ...
            server.hackDifficulty = server.minDifficulty
            //calculate timings
            this.calculate_timings(ns, server, player, formulas_available)
            //calculate threads
            this.calculate_threads(ns, server, player, formulas_available)
            //debug
    ...
    */
    calculate_threads(ns, server, player, formulas_available) {
        ...
        if (formulas_available) {           
            ...
            for (const index = 1; index <= max_hack_threads; index++) {
                ...
                //calc ram cost
                const ram_cost_best = (index * CONSTANTS.RAM.WORKER.HACK) + (threads_grow * CONSTANTS.RAM.WORKER.GROW) + ((threads_weaken_1 + threads_weaken_2) * CONSTANTS.RAM.WORKER.WEAKEN)
