import { port_darknet_information, port_darknet_password, port_darknes_servers_done } from "scripts/constants.js"
...
async function scan_servers(ns) {
    ...
    for (const server_darknet_hostname of servers_darknet_hostnames) {
        ...
        const result, logs = await ns.dnet.heartbleed(server_darknet_hostname) //: HeartbleedOptions): Promise<DarknetResult & { logs: string[] }>;
...
async function authenticate_servers(ns, servers_found) {
    ...
    for (const darknet_hostname of servers_found) {
        ...
        if (password_map.has(darknet_hostname)) {
            ...
            //try to authenticate
            var result = await ns.dnet.authenticate(darknet_hostname, password)
            //if successfull
            if (result.success) {
                //get the results
